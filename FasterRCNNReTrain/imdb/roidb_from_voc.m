...
try
    ...
    if isempty(regions)
        ...
        if imdb.flip
            regions.images = imdb.image_ids(1:8:end);
        else
    ...
    if ~imdb.flip
        ...
        for i = 1:length(imdb.image_ids)/8
            tic_toc_print('roidb (%s): %d/%d\n', roidb.name, i, length(imdb.image_ids)/2);
            try
                voc_rec = PASreadrecord(sprintf(VOCopts.annopath, imdb.image_ids{(i-1)*8+1}));
            catch
                voc_rec = [];
            end
            if ~isempty(regions)
                [~, image_name1] = fileparts(imdb.image_ids{(i-1)*8+1});
                [~, image_name2] = fileparts(regions.images{i});
                assert(strcmp(image_name1, image_name2));
                assert(imdb.flip_from(i*8-3) == i*2-1);
            end
            %% roi calculation
            for k = 1 : 8
                roidb.rois((i - 1) * 8 + k) = attach_proposals(voc_rec, regions.boxes{i}, imdb.class_to_id, opts.exclude_difficult_samples, k);
            end
        end
    end
...
% ------------------------------------------------------------------------
function rec = attach_proposals(voc_rec, boxes, class_to_id, exclude_difficult_samples, flip_index)
% ------------------------------------------------------------------------

% change selective search order from [y1 x1 y2 x2] to [x1 y1 x2 y2]
if ~isempty(boxes)
    boxes = boxes(:, [2 1 4 3]);
    boxes = calc_proposals(boxes, voc_rec.imgsize, flip_index);
end
...
if isfield(voc_rec, 'objects')
    ...
    gt_boxes = calc_proposals(gt_boxes, voc_rec.imgsize, flip_index);
    %% show image to check if box calculation is correct
%     append = {'', '_2', '_3', '_4', '_flip', '_flip_2', '_flip_3', '_flip_4'};
%     arr = strsplit(voc_rec.filename, '.');
%     imgpath = '/home/cnn/faster_rcnn/datasets/VOCdevkit/VOC2012/JPEGImages/%s.jpg';
%     im = imread(sprintf(imgpath, sprintf('%s%s', arr{1}, append{flip_index})));
%     figure(flip_index);
%     imshow(im);
%     linewidth = 2;
%     for i = 1 : size(gt_boxes, 1)
%         rectangle('Position', RectLTRB2LTWH(gt_boxes(i, :)), 'LineWidth', linewidth, 'EdgeColor', [0 1 0]);
%     end
    %%
...
% elseif flip_index == 4
%     % rot90
%     boxes = cat(2, boxes(:, 2), imgsize(2) + 1 - boxes(:, 3), boxes(:, 4), imgsize(2) + 1 - boxes(:, 1));
% elseif flip_index == 5
%     % fliplr
%     boxes(:, [1, 3]) = imgsize(1) + 1 - boxes(:, [3, 1]);
% elseif flip_index == 6
%     %
%     boxes = cat(2, imgsize(2) + 1 - boxes(:, 3), imgsize(1) + 1 - boxes(:, 4), imgsize(2) + 1 - boxes(:, 1),  imgsize(1) + 1 - boxes(:, 2));
% elseif flip_index == 7
%     %
%     boxes = cat(2, imgsize(2) + 1 - boxes(:, 3), boxes(:, 2), imgsize(2) + 1 - boxes(:, 1), boxes(:, 4));
% elseif flip_index == 8
%     %
%     boxes = cat(2, imgsize(1) + 1 - boxes(:, 4), imgsize(2) + 1 - boxes(:, 3), imgsize(1) + 1 - boxes(:, 2),  imgsize(2) + 1 - boxes(:, 1));
% end
