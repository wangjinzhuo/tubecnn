...
%% -------------------- TESTING --------------------
disp('test up');
wjz_root='/media/cnn/NANNANLI/ucf_11';
wjz_save_path = '/home/cnn/wjz_boxes_save_ucf_11_vgg';
d = dir(wjz_root);

for i = 3 : length(d), 

tic       
    if ~exist([wjz_root '/' d(i).name], 'dir'),
        continue;
    end
    dd = dir([wjz_root '/' d(i).name]);
    mkdir([wjz_save_path '/' d(i).name]);
    
    for ii = 3 : length(dd),
        ddd = dir([wjz_root '/' d(i).name '/' dd(ii).name]);
        mkdir([wjz_save_path '/' d(i).name  '/' dd(ii).name]);       
        
        for iii = 3 : length(ddd),
            if exist([wjz_root '/' d(i).name '/' dd(ii).name '/' ddd(iii).name], 'dir'),
                continue;
            end
            im_name = ddd(iii).name;
            if ~strcmp(im_name(length(im_name)-3:end),'.jpg'),
                continue;
            end
            im = imread([wjz_root '/' d(i).name '/' dd(ii).name '/' im_name]);     
            if opts.use_gpu
                im = gpuArray(im);
            end
             % test proposal
...
%             pause(0.1);   
            a = [wjz_save_path '/' d(i).name '/' dd(ii).name];
            save([a '/' im_name(1:length(im_name)-4) '.mat'],'boxes_cell');
        end
    end
toc
end
