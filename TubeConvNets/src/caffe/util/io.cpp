...
bool ReadSegmentRGBToDatum(const string& filename, const int label,
    ...
    const string& img_root, const string& tube_root, bool tube) {
	cv::Mat cv_img;
	string* datum_string;
	char tmp[30];
	int cv_read_flag = (is_color ? CV_LOAD_IMAGE_COLOR :
	...
	for (int i = 0; i < offsets.size(); ++i){
		...
		for (int file_id = 1; file_id < length+1; ++file_id){
			...
			string filename_t = img_root + filename + "/" + tmp;
			// LOG(ERROR) << filename_t;
...
bool ReadSegmentFlowToDatum(const string& filename, const int label,
	...
	for (int i = 0; i < offsets.size(); ++i){
		...
		for (int file_id = 1; file_id < length+1; ++file_id){
			...
			vector<int*> tube_locations;
			if (tube) {
				// read tube file
				sprintf(tmp,"image_%04d.txt", int(file_id + offset));
				string tube_filename = tube_root + filename + "/" + tmp;
				std:: ifstream infile(tube_filename.c_str());
				float top_x, top_y, bot_x, bot_y, score;
				
				// tube file format:
				// top_y top_x bot_y bot_x score
				while (infile >> top_y >> top_x >> bot_y >> bot_x >> score) {
					int loc[4];
					loc[0] = int(top_x);
					loc[1] = int(top_y);
					loc[2] = int(bot_x);
					loc[3] = int(bot_y);
					tube_locations.push_back(loc);
				}
			}
			
			if (tube && tube_locations.size() != 0)
			{
				// flow_x
				for (int h = 0; h < cv_img_x.rows; ++h) {
					for (int w = 0; w < cv_img_x.cols; ++w) {
						// check whether point(w, h) is in any tube
						bool in_tubes = false;
						for (int tube_index = 0; tube_index < tube_locations.size(); tube_index ++) {
							int top_x = tube_locations[tube_index][0];
							int top_y = tube_locations[tube_index][1];
							int bot_x = tube_locations[tube_index][2];
							int bot_y = tube_locations[tube_index][3];

							if (h >= top_y && h <= bot_y && w >= top_x && w <= bot_x) {
								in_tubes = true;
								break;
							}
						}
						...
						if (in_tubes) {
							...
							datum_string->push_back(static_cast<char>(uchar(128)));
						}
					}
				}
				// flow_y
				for (int h = 0; h < cv_img_y.rows; ++h) {
					for (int w = 0; w < cv_img_y.cols; ++w) {
						...
						if (in_tubes) {
							int a = int(cv_img_y.at<uchar>(h,w));
